...
describe('Pick Up Order [UNIT]', () => {
    beforeEach(() => {
        inMemoryRecipientRepository = new InMemoryRecipientRepository()
        inMemoryDeliveryPersonRepository = new InMemoryDeliveryPersonRepository()
        inMemoryOrderRepository = new InMemoryOrderRepository()
        sut = new PickUpOrderUseCase(inMemoryOrderRepository, inMemoryDeliveryPersonRepository)
    })

    it('should be able to pick up a order', async() => {
        ...
        expect(inMemoryOrderRepository.items[0].pickupDate).not.toBeNull()
    })
